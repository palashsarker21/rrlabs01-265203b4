...
export function SocialLinks({
  ...
  if (variant === "list") {
    return (
      <ul aria-label={ariaLabel} className={cn("grid gap-2 sm:grid-cols-2", className)}>
        {profiles.map((p) => {
  ...
  return (
    <ul aria-label={ariaLabel} className={cn("flex flex-wrap items-center gap-3", className)}>
      {profiles.map((p) => {
        const Icon = PLATFORM_ICON[p.platform]!;
        return (
          <li key={p.platform}>
            <a
              ...
              rel="noopener noreferrer"
              onClick={() =>
                trackEvent("social_link_click", { component: "social-icons", platform: p.platform })
              }
              title={`${p.label} — opens in a new tab`}
              aria-label={`${p.label} (opens in a new tab)`}
